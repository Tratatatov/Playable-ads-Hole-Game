...
function parseAlign(val, map, def) {
    return val && map[val] !== undefined ? map[val] : def;
}
// ─────────────────────────────────────────────
// Node builder
// ─────────────────────────────────────────────
function applyBaseTransform(node, def) {
    // UITransform
...
function applyWidget(node, widgetDef) {
    if (!widgetDef)
        return;
...
function applyLayout(node, layoutDef) {
    if (!layoutDef)
        return;
...
function buildNode(def, parentNode) {
    ...
    switch (def.type) {
        case 'Sprite': {
            ...
            if (def.spritePath) {
                cc.assetManager.loadAny({ path: def.spritePath.replace(/\.[^/.]+$/, ''), type: cc.SpriteFrame }, (err, sf) => {
                    if (!err && sf) {
...
exports.methods = {
    ...
    buildNodes(layout) {
        try {
            ...
            rootTransform.setContentSize(layout.canvasWidth ?? 1080, layout.canvasHeight ?? 1920);
            // Build all nodes
            let count = 0;
